...
struct MPIUtils {
     ...
     */
    static int myRank(MPI::Intracomm & comm=MPI::COMM_WORLD);

    /*! \brief Wrapps MPI_COMM_SIZE
     *  \param comm: The communicator to use.
     *  \return: The sise of the communicator (the total number of processes).
     */
    static int size(MPI::Intracomm & comm=MPI::COMM_WORLD);

    /*! \brief Wrapps MPI_BARRIER, syncronizing processes.
     *  \param comm: The communicator to use.
     */
    static void barrier(MPI::Intracomm & comm=MPI::COMM_WORLD);

    /*! \brief Returns true if the calling process is the master.
     */
    static bool isMaster(MPI::Intracomm & comm=MPI::COMM_WORLD)
    { return (myRank(comm) == 0); }

    /*! \brief Split a size to different chunks.
     */
    static std::pair<int, int> splitSize(int size, MPI::Intracomm & comm=MPI::COMM_WORLD);
